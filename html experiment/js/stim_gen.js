...
// Similar code to Pilot 2, but recording all 96 trials for creating a database
...
var chaserMiniBlock_20chasing = [0, 0, 0, 0, 0, 1]
var chaserMiniBlock_80chasing = [1, 1, 1, 1, 1, 0]
var chaserMiniBlock_50chasing = [1, 1, 1, 0, 0, 0]

var chasingTrialsShuffled = []
var volatileEnvironmentFirst = Math.random()>0.5
var withinVolatileEnvironment_80first = Math.random()>0.5
...
var volatilityTracker = []
var miniBlockTracker = []

for (i = 0; i < N_practice_trial; i++) {
	volatilityTracker.push("practice")
	miniBlockTracker.push("practice")
}

var environmentSwitchAfterTrial = 24

if (volatileEnvironmentFirst) {
	if(withinVolatileEnvironment_80first) { 
		chasingTrialsShuffled =  chasingTrialsShuffled.concat(jsPsych.randomization.repeat(chaserMiniBlock_80chasing, 1));
		chasingTrialsShuffled =  chasingTrialsShuffled.concat(jsPsych.randomization.repeat(chaserMiniBlock_80chasing, 1));
		chasingTrialsShuffled =  chasingTrialsShuffled.concat(jsPsych.randomization.repeat(chaserMiniBlock_80chasing, 1));
		chasingTrialsShuffled =  chasingTrialsShuffled.concat(jsPsych.randomization.repeat(chaserMiniBlock_80chasing, 1));
		for (i = 0; i < environmentSwitchAfterTrial; i++) {
			miniBlockTracker.push("miniBlockChasingIsDominant_Yes")
		}
		...
		chasingTrialsShuffled =  chasingTrialsShuffled.concat(jsPsych.randomization.repeat(chaserMiniBlock_20chasing, 1));
		for (i = 0; i < environmentSwitchAfterTrial; i++) {
			miniBlockTracker.push("miniBlockChasingIsDominant_Not")
		}
	} else {
		chasingTrialsShuffled =  chasingTrialsShuffled.concat(jsPsych.randomization.repeat(chaserMiniBlock_20chasing, 1));
		chasingTrialsShuffled =  chasingTrialsShuffled.concat(jsPsych.randomization.repeat(chaserMiniBlock_20chasing, 1));
	...
	for (i = 0; i < (N_test_Trial/2); i++) {
		volatilityTracker.push("0")
		miniBlockTracker.push("NaN")
	}
...
var subjectId = jsPsych.randomization.randomID(8);
	jsPsych.data.addProperties({subject: subjectId, prolificID: prolific_PID, proj_name: ProjName, exp_version: expVersion, volatileEnvironmentFirst: volatileEnvironmentFirst, withinVolatileEnvironment_80first:withinVolatileEnvironment_80first});
...
var test_trial = {       
    ...
    trial_duration: 5000, // or your trial length in ms
    on_load: function() {
        // fullscreen
        ...
        // start trial display
        display(0); // your existing function
        ...
        recorder.onstop = () => {
            ...
            a.href = url;
            a.download = "trial_" + jsPsych.data.get().last(1).trials.length + ".webm"; // unique name
            a.click();
        };
        recorder.start();
